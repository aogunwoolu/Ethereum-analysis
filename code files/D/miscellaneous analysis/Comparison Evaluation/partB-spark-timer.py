...
'''
method for timing how long each run takes and taking the average for comparison
'''
def timer(func, nameOfTest):
    times = []
    for i in range(6):
        start = time()
        func()
        end = time()
        lengthOfJob = end - start
        times.append(lengthOfJob)
        print("--------------------------------------------------------------------\n\t\t\tattempt {attemptno}\n--------------------------------------------------------------------\n{name}: {timed}\n--------------------------------------------------------------------".format(name = nameOfTest, timed = lengthOfJob, attemptno = i + 1))

    print("--------------------------------------------------------------------\n\t\t\taverage time: {average}\n--------------------------------------------------------------------".format(average=sum(times)/5))
...
'''
running the method on command call
'''
timer(spark_B, "spark run")
